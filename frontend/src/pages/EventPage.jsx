...
const EventPage = ({ slug: slugProp }) => {
  const params = useParams();
  const slug = slugProp || params.slug;
  const data = findEventPage(slug);
  ...
  const relatedPages = (() => {
    ...
    (data.related || []).forEach((s) => {
      const e = findEventPage(s);
      if (e && !seen.has(e.slug)) { out.push({ ...e, tag: 'Event Transportation' }); seen.add(e.slug); }
    });
    for (const r of ROUTE_PAGES) {
      if (out.length >= 6) break;
      if (!seen.has(r.slug)) { out.push({ ...r, tag: 'Airport Transfer' }); seen.add(r.slug); }
    }
    return out.slice(0, 6);
  })();

  return (
    <div className="min-h-screen bg-white" data-testid={`event-page-${data.slug}`}>
      {/* HERO */}
      <section
      ...
      >
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <Link to="/booking">
              <Button className="bg-amber-500 hover:bg-amber-400 text-black font-bold text-base sm:text-lg px-8 py-6 shadow-lg w-full sm:w-auto" data-testid="event-cta-book">
                Get a Free Quote <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
            </Link>
            <a href={PHONE_TEL}>
              <Button variant="outline" className="border-2 border-amber-500 text-amber-400 hover:bg-amber-500 hover:text-black font-bold text-base sm:text-lg px-8 py-6 w-full sm:w-auto" data-testid="event-cta-call">
            ...
            <aside className="lg:col-span-2">
              <Card className="border-2 border-amber-500 shadow-xl sticky top-28">
                <CardContent className="p-6 sm:p-8">
                  <h3 className="text-xl font-bold text-gray-900 mb-2">Book {data.name} Transportation</h3>
                  <p className="text-base font-semibold text-gray-900 mb-1">Contact us for pricing</p>
                  <p className="text-sm text-gray-600 mb-5">Get a fast, exact quote for your event date and vehicle.</p>
                  <Link to="/booking" className="block">
                    <Button className="w-full bg-amber-500 hover:bg-amber-400 text-black font-bold py-6 text-base mb-3" data-testid="event-sidebar-book">
                      Get a Free Quote
                    </Button>
                  </Link>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-8 text-center">
              {data.name} Transportation — Frequently Asked Questions
            </h2>
            <div className="space-y-4">
              {data.faqs.map((f) => (
                <div key={f.q} className="border border-gray-200 rounded-xl p-5 sm:p-6" data-testid="event-faq">
            ...
            <div className="text-center mt-8">
              <Link to="/booking">
                <Button className="bg-amber-500 hover:bg-amber-400 text-black font-bold px-8 py-6 text-base">
                  Book your {data.name} transportation <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </Link>
            </div>
          </div>
      ...
      <section className="py-16 sm:py-20 bg-gradient-to-b from-gray-50 to-white">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-10 text-center">More Event & Airport Transportation</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {relatedPages.map((r) => (
                <Link key={r.slug} to={`/${r.slug}`} className="group" data-testid={`related-event-${r.slug}`}>
                  <Card className="border border-gray-200 hover:border-amber-500 hover:shadow-xl transition-all duration-300 h-full">
                    <CardContent className="p-5 sm:p-6">
                      <span className="text-xs text-amber-600 font-semibold uppercase tracking-wider">{r.tag}</span>
      ...
      <section className="py-16 sm:py-20 bg-gradient-to-br from-black via-gray-900 to-black text-white">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">
            Book your <span className="text-amber-400">{data.name}</span> transportation today
          </h2>
          <p className="text-base sm:text-xl text-gray-300 max-w-2xl mx-auto mb-8">
            Flat rate locked at booking, professionally chauffeured, with a staged pickup when the event ends — free cancellation up to 12 hours before pickup for special events. Contact us for a free, exact quote.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link to="/booking">
              <Button className="bg-amber-500 hover:bg-amber-400 text-black font-bold text-base sm:text-lg px-8 py-6 shadow-lg w-full sm:w-auto" data-testid="event-final-book">
                <Calendar className="mr-2 h-5 w-5" /> Get a Free Quote
              </Button>
            </Link>
            <a href={PHONE_TEL}>
              <Button variant="outline" className="border-2 border-amber-500 text-amber-400 hover:bg-amber-500 hover:text-black font-bold text-base sm:text-lg px-8 py-6 w-full sm:w-auto" data-testid="event-final-call">
