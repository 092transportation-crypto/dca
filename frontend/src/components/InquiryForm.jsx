...
const InquiryForm = () => {
  ...
  // Latest state broadcast by the QuoteCalculator above the form, so the
  // quoted price rides along with the submission email.
  ...
  useEffect(() => {
    ...
    window.addEventListener('dca:quote-state', onState);
    window.addEventListener('dca:quote-apply', onApply);
    return () => {
      window.removeEventListener('dca:quote-state', onState);
      window.removeEventListener('dca:quote-apply', onApply);
    };
  }, []);
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    // Attach the calculator's quote only if it matches the trip being booked.
    ...
    try {
      const res = await fetch('/api/quote-requests', {
        ...
        body: JSON.stringify({
          ...
          ...(calc?.tripType ? { trip_type: calc.tripType } : {}),
