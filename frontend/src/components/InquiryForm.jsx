...
// Trip type drives the instant-quote calculator: point-to-point trips are
// priced live from mileage; hourly and special/event get a custom quote.
const TRIP_TYPES = [
...
const InquiryForm = () => {
  ...
  // Instant-quote distance lookup: idle | loading | ready | error
  ...
  // Debounced driving-distance lookup once both addresses are in.
  const pickupTrimmed = form.pickup_location.trim();
  const dropoffTrimmed = form.dropoff_location.trim();
  useEffect(() => {
    if (form.trip_type !== 'Point-to-Point') return undefined;
  ...
  }, [form.trip_type, pickupTrimmed, dropoffTrimmed]);

  const quote = useMemo(
    () =>
      form.trip_type === 'Point-to-Point' && distance.status === 'ready'
        ? computeQuote(distance.miles, form.vehicle_type)
        : null,
    [form.trip_type, form.vehicle_type, distance]
  );
    ...
    try {
      const res = await fetch('/api/quote-requests', {
        ...
        body: JSON.stringify({
          ...
          // Instant point-to-point estimate shown to the customer, so the
          // team sees exactly what was quoted.
          ...(quote
            ? {
                instant_quote: {
                  miles: quote.miles,
      ...
      >
        ...
        >
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            ...
            {/* Trip Type — drives the instant quote calculator */}
            <motion.div variants={itemVariants} className="md:col-span-2">
              <span className={groupLabel}>
                Trip Type
                <span className="ml-2 normal-case tracking-normal text-gray-500">— point-to-point gets an instant quote</span>
              </span>
              <div className="flex flex-wrap gap-2">
                {TRIP_TYPES.map(({ value, icon: Icon }) => {
                  ...
                  return (
                    <motion.button
                      key={value}
                      type="button"
                      data-testid={`inquiry-trip-${value.toLowerCase().replace(/[^a-z]+/g, '-')}`}
                      aria-pressed={active}
                      whileTap={{ scale: 0.94 }}
          ...
          {/* Instant quote panel */}
          <motion.div variants={itemVariants} className="mt-6" data-testid="inquiry-quote-panel">
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/[0.05] p-5">
              <div className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.14em] text-amber-400">
                <Calculator size={14} />
                {form.trip_type === 'Point-to-Point' ? 'Instant Quote' : 'Pricing'}
              </div>

              {form.trip_type !== 'Point-to-Point' ? (
                ...
                <p className="text-sm text-gray-300">
                  We couldn&apos;t calculate the distance for those addresses —
                  submit your request and we&apos;ll follow up with an exact
                  quote.
                </p>
              ) : !form.vehicle_type ? (
                <p className="text-sm text-gray-300">
                  <span className="tabnums font-semibold text-white">
                ...
                <p className="text-sm text-gray-300" data-testid="inquiry-quote-custom">
                  Custom pricing for {form.vehicle_type} — submit your request
                  and we&apos;ll follow up with a quote.
                </p>
              ) : (
                <div data-testid="inquiry-quote-breakdown">
                  <dl className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3 border-t border-amber-500/25 pt-2.5">
                      ...
                      <dd
                        className="tabnums text-lg font-bold text-amber-400"
                        data-testid="inquiry-quote-total"
                      >
                        {money(quote.total)}
                      </dd>
                    </div>
                  ...
                  <p className="mt-3 text-[11px] leading-relaxed text-gray-500">
                    Instant estimate for point-to-point trips. Your chauffeur
                    and final confirmation follow after you submit — no payment
                    is taken now.
                  </p>
                </div>
              )}
